...
def validar_nova_pagina():
    with sync_playwright() as p:
        ...
        # Clica no botão salvar sem preencher nada
        page.get_by_role("button", name="Salvar").click()
        
        # Valida se a mensagem de erro vai ser exibida e com o texto esperado
        mensagem_erro = page.locator('#span-errors')
        expect(mensagem_erro).to_be_visible()
        expect(mensagem_erro).to_have_text('Existem campos obrigatórios não preenchidos.')
